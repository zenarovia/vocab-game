...
- Matching modality only (word ↔ digit), Batch 1 content (numbers 0–10)
...
- Adaptive-lite word weighting — words you get wrong resurface more often
  in the next round (a lightweight stand-in for the full per-word adaptive
  engine, which needs a real backend/database to track history across
  sessions and students)
- Coin economy — matches earn coins, streaks earn bonus coins
- Tab-switch detection — leaving the tab still lets the round resolve
  (participation), but flags that round's matches as not counting toward
...
- The other 4 modalities (typing, listening, fill-in-context, speed challenge)
